 ...
 * Cena de comer muito
 *
 * Unica diferenca(em relacao a Cena de comer) e a quantidade de moscas na tela
 *
 * @author Erik
 * @version 1.0.0
 */
public class EatALotScene extends Scene
{
    ...
    /**
     * Desenha as moscas na tela baseado no frame em que esta
     ...
     *
     * @param bug Ponto atual das moscas
     * @param frame Frame onde sera carregada
    ...
    private void drawBug(Actor bug, Frame frame, int frameIndex){
        ...
        // para cada uma das linhas das moscas
        for(int j=0;j<=2;j++){
            if(bug.getRow()+j>=data.size()) break;

            char rowData[] = data.get(bug.getRow()+j).toCharArray();
            int i = j!=1?-5:-4; // gera o xadrez na imagem
            for(;i<=5;i+=2) // carrega a informacao na linha
